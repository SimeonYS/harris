...
class HarrisPipeline:
    ...
    # Database setup
    conn = sqlite3.connect('harris.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `harris`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM harris WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO harris (date, title, link, content)"
